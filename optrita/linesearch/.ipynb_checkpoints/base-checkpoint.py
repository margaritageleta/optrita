...
# Newton method
#------------------------------------------------------------------------------
def newton(x, f, g, h, BLS_params, eps = 1e-6, kmax = 1500, precision = 6):
    """
    Parameters
    ----------
    x : initial point
    ...
    k = 0
    while np.linalg.norm(g(x)) > eps and k < kmax:
        lamb, bibi = np.linalg.eig(h(x.squeeze()))
        print(lamb, "\t")
        d = -np.linalg.inv(h(x.squeeze())) @ g(x)
        x = x + d
        k += 1
        gradient_norm = round(np.linalg.norm(g(x)), precision)
        info.append([alpha, gradient_norm])
...
#------------------------------------------------------------------------------
def positify_hessian(H, delta = 0.01):
    """
    Paramters
    ---------
    H : matrix
    ...
    """
    lambdas, Q = np.linalg.eig(H)
    lambdas = lambdas.clip(delta)
    return Q @ np.diag(lambdas) @ Q.T

def MNSD(x, f, g, h, BLS_params, eps = 1e-6, kmax = 1500, precision = 6):
    """
    Parameters
    ----------
    x : initial point
    ...
    print("[MN-SD] Initial:", x.T)
    k = 0
    while np.linalg.norm(g(x)) > eps and k < kmax:
        print(h(x.squeeze()), "\t")
        B = positify_hessian(h(x.squeeze()))
        print(B, "\t")
        d = -np.linalg.inv(B) @ g(x)
        alpha, _ = BLS(x, d, f, g, **BLS_params)
        x = x + alpha*d
        k += 1
    ...
    # ================================================================================================ #
    # OUTPUT:
    print("[MN-SD] Final:",x.T)
    print("[MN-SD] Iterations:", k)
    data = pd.DataFrame(info, columns=["alpha", "||g(x)||"], dtype=np.float)
    return Xk, data
...
def CMI(H, t = 0, k = 0): # recursive
    ...
    Returns
    -------
    Factorized matrix H with t factor at iteration k
    ...
    try:
        ...
        # print("⚠️ Matrix is not positive definite at it", k)
        l_UB = np.linalg.norm(H, ord='fro')
        k = k + 1
        t = (1.01 - 1/2**k)*l_UB
        return CMI(H + t*np.eye(len(H)), t, k) 
    
def MNCF(x, f, g, h, BLS_params, eps = 1e-6, kmax = 1500, precision = 6):
    """
    Parameters
    ----------
    x : initial point
    ...
    # ================================================================================================ #
    # OUTPUT:
    print("[MN-CF] Final:",x.T)
    print("[MN-CF] Iterations:", k)
    data = pd.DataFrame(info, columns=["alpha", "||g(x)||"], dtype=np.float)
    return Xk, data
